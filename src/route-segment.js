...
(function (angular) {
    ...
    mod.provider("$routeSegment", ["$routeProvider", function ($routeProvider) {
        ...
        function pointer(segment, parent) {
            ...
            return {
                ...
                within: function (childName) {
                    ...
                    var child = segment[camelCase(childName)];
                 ...
                 function ($rootScope, $q, $http, $templateCache, $route, $routeParams, $injector) {
                     ...
                     */
                         getSegmentUrl: function (segmentName, routeParams) {
                             ...
                             let url = segmentRoutes[segmentName];
                             for (let param in routeParams) {
                                 if (!routeParams.hasOwnProperty(param)) {
                                     return;
                                 }
                                 const regexp = new RegExp("\:" + param + "[\*\?]?", "g");
                                 url = url.replace(regexp, routeParams[param]);
                             }
                             url = url.replace(/\/\:.*?\?/g, "");

                             let match = url.match(/\/\:([^\/]*)/);
                             if (match)
                                 throw new Error("Route param `" + match[1] + "` is not specified for route `" + segmentRoutes[segmentName] + "`");
                ...
                // When a route changes, all interested parties should be notified about new segment chain
                     $rootScope.$on("$routeChangeSuccess", function (event, args) {
                         ...
                         if (route && route.segment) {
                             ...
                             for (let i = 0; i < segmentNameChain.length; i++) {
                             ...
                             curSegmentPromise.then(function () {
                                ...
                                // Removing redundant segment in case if new segment chain is shorter than old one
                                 if ($routeSegment.chain.length > segmentNameChain.length) {
                                     ...
                                     for (let i = segmentNameChain.length; i < oldLength; i++) {
                     ...
                     function updateSegment(index, segment) {
                         ...
                         if ($rootScope.$broadcast("routeSegmentChangeStart", index, segment)
                        .defaultPrevented) {
                        ...
                        .then(
                            ...
                            function (error) {
                                ...
                                if (params.resolveFailed) {
                                    ...
                                    return resolve(index, name, angular.extend({
                                        resolve: newResolve
                     ...
                     function broadcast(index) {
                         ...
                         $routeSegment.name = "";
                         for (let i = 0; i < $routeSegment.chain.length; i++)
                     ...
                     function getSegmentInChain(segmentIdx, segmentNameChain) {
                         ...
                         for (let i = 0; i <= segmentIdx; i++) {
                             ...
                             if (curSegment[camelCase(nextName)])
